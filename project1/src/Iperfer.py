...
# Create server
ServerName = input("Enter Hostname ")
if ServerName == 0 or ServerName == '':
    print("Error: missing or additional arguments")
    sys.exit()

ServerIP = socket.gethostbyname(ServerName)
ServerPort = input("Enter Port from range 1024 - 65535: ") #takes in port
if ServerPort == '':
    print("Error: missing or additional arguments")
    sys.exit()
...
sendTime = input("Enter time in seconds ")
if sendTime == '':
    print("Error: missing or additional arguments")
    sys.exit()
...
while 1:
    ...
    elp = end - start                #total time elepssed

    if elp >= float(sendTime):       #if the elepssed time is greater than time usr entered end the count and break the loop
        break

   # else:
   # continue
#print(str("BYTES: " + str(byt) + "/" + str(elp) + "time")) #print total bytes sent + time elepssed
